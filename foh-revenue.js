...
function revYearRow(p){
  ...
  if(kind==='monthly'||kind==='closed'){
    ...
    return {kind:kind, net:Number(r.net_actual)||0, budget:mb, toDate:false, days:null,
  ...
  return {kind:kind, net:null, budget:null, toDate:false, days:null, fnb:null, source:null};
}
  ...
  if(mks.length){
    ...
    Object.keys(byY).sort().forEach(function(y){
      ...
      byY[y].forEach(function(k){
        ...
        if(r.status==='closed'){ cl++; line.push(mn+' CLOSED'); return; }
        var n=Number(r.net_actual)||0; yn+=n; ct++;
        line.push(mn+' '+Math.round(n)+(r.food_net!=null?(' (food '+Math.round(r.food_net)+', bev '+Math.round(r.bev_net||0)+', tob '+Math.round(r.tobacco_net||0)+')'):''));
      });
      L.push('  '+y+': '+line.join(' | '));
...
function revRenderMonthlyOnly(p){
  ...
  h.push('<div class="rev-card"><div class="rev-k">Net sales &mdash; month</div><div class="rev-v">'+revMoney(net)+'</div><div class="rev-sub">'+revMonthLabel(p)+'</div></div>');
  if(hasSplit){
...
function revRenderYear(){
  ...
  for(var mo=1; mo<=12; mo++){
    ...
    if(row.kind==='closed'){
      counted++; closedCount++;
      h.push('<tr class="rev-sun"'+click+'><td class="rev-day rev-mut">'+name+'</td><td colspan="6" class="rev-mut">Closed &mdash; the venue did not trade'+(row.note?'':'')+'</td></tr>');
      continue;
    }
    counted++;
