...
async function stLoadPrevMonth(){
  ...
  // Scoped to stDept like everything else, so each section compares against its
  ...
  stChannel = sb.channel('stock_take_'+stDept+'_'+stMonth)
    ...
    .on('postgres_changes', { event:'UPDATE', schema:'public', table:'stock_take_sheets', filter:'month=eq.'+stMonth },
      function(payload){
        ...
        var r = payload.new; if(!r || r.dept!==stDept || !stSheet || r.id!==stSheet.id) return;
        var wasLocked = !!stSheet.locked;
        stSheet = r;
        if(!!r.locked!==wasLocked && stActive()) stRender();
      })
    .subscribe();
}
...
async function stSetQty(itemId, value){
  if(!stUser) return;
  if(stIsLocked()){ toast('This month is locked — counts can no longer be changed.', true); stUpdateRowUI(itemId); return; }
...
async function stAddQty(itemId, value){
  if(!stUser) return;
  if(stIsLocked()){ toast('This month is locked — counts can no longer be changed.', true); var b=document.getElementById('st-add-'+itemId); if(b) b.value=''; return; }
...
function stRender(){
  ...
  root.innerHTML =
    ...
    '<div class="st-toolbar">'+
      ...
      '<button class="st-select" id="st-cat-btn" onclick="stShowCatFilter()" style="text-align:left;cursor:pointer">'+stEsc(stCatLabelText())+' ▾</button>'+
...
function stShowCatFilter(){
  ...
  box.innerHTML='<div style="background:#fff;border-radius:12px;padding:18px;width:90%;max-width:340px;max-height:80vh;display:flex;flex-direction:column" onclick="event.stopPropagation()">'+
    ...
    '<div style="display:flex;gap:8px;justify-content:space-between;margin-top:12px">'+
      '<button class="st-btn" onclick="stCatFilterSetAll(this)">Clear (show all)</button>'+
      '<div style="display:flex;gap:8px">'+
        '<button class="st-btn" onclick="document.getElementById(\'st-catf-modal\').remove()">Cancel</button>'+
        '<button class="st-btn" onclick="stApplyCatFilter(this)">Apply</button>'+
      '</div>'+
    '</div></div>';
...
async function stSetDept(dept){
  ...
  stDept = dept; stSearch=''; stCatFilters=[]; stUnitSel={};
  await stOpen();
}

// ── lock / unlock a finalized month: once locked, nobody can enter, add-to,
// clear, add-missing-item, or re-upload over this dept+month. Email/Excel/Print
...
function stShowAdd(){
  if(!stUser){ toast('Enter your employee ID first.', true); return; }
  if(stIsLocked()){ toast('This month is locked — no items can be added.', true); return; }
...
//
// ALWAYS ONE SECTION AT A TIME: everything here reads the current stDept, so
// Wine is compared against Wine's own last month. Switching section re-runs
// stOpen(), which reloads the previous month for that section.
//
...
// per-item current + previous for the CURRENT section, unioned across both months
function stCompareRows(){
  ...
  stItems.forEach(function(it){
    ...
    var c = stCounts[it.id];
    if(c && c.qty!=null){ e.qty += Number(c.qty)||0; e.val += stLineValue(it); e.counted = true; }
  });
  var keys = {};
...
function stShowCompare(){
  if(!stCanCompare()){ toast('The month comparison is for the cost controller\'s code (0000).', true); return; }
...
function stRenderCompare(){
  ...
  var body = shown.length
    ...
    : '<div class="st-nodata">Nothing moved by AED '+(Number(stCmpMinAed)||0).toLocaleString('en-US')+' or more in '+stEsc(stDeptLabel())+'. Lower the threshold, or untick "flagged only" to see every item.</div>';

  el.innerHTML = head + body;
}
function stCmpSetMin(v){
  var n = Number(String(v||'').replace(',','.'));
  if(isNaN(n) || n<0) toast('"'+v+'" is not a number — threshold left at AED '+stCmpMinAed+'.', true);
  else stCmpMinAed = n;
  stRenderCompare();
}
...
async function stCompareExcel(){
  try{
    ...
    ws['!cols'] = [{wch:8},{wch:20},{wch:42},{wch:16},{wch:12},{wch:14},{wch:12},{wch:14},{wch:14},{wch:10},{wch:26}];
    var wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Comparison');
    XLSX.writeFile(wb, "Roberto's "+stDeptLabel()+" Stock Take comparison "+stPrevMonth+" vs "+stMonth+".xlsx");
  }catch(e){ toast('Could not build the comparison Excel: '+e.message, true); }
}
...
async function stApplyUpload(month, items, filename){
  ...
  var dC=await sb.from('stock_take_counts').delete().eq('venue_id',STOCK_VENUE).eq('dept',stDept).eq('month',month);
  if(dC.error) throw new Error('Could not clear old counts: '+dC.error.message);
  var dI=await sb.from('stock_take_items').delete().eq('venue_id',STOCK_VENUE).eq('dept',stDept).eq('month',month);
  if(dI.error) throw new Error('Could not clear old items: '+dI.error.message);
  var dS=await sb.from('stock_take_sheets').delete().eq('venue_id',STOCK_VENUE).eq('dept',stDept).eq('month',month);
  if(dS.error) throw new Error('Could not clear old sheet: '+dS.error.message);
  var iS=await sb.from('stock_take_sheets').insert({ venue_id:STOCK_VENUE, dept:stDept, month:month, status:'counting', source_filename:filename, item_count:items.length, uploaded_by:stUser.emp_id, uploaded_by_name:stUser.name });
